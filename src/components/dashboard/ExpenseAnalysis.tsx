...
const ExpenseAnalysis: React.FC<ExpenseAnalysisProps> = ({
  ...
  const filteredExpenses = expenses.filter(expense => {
    ...
    if (selectedMonth || selectedYear) {
      ...
      matchesDate = (!selectedMonth || expenseMonth === selectedMonth) &&
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.orderId || !formData.customerName || !formData.orderType || !formData.date) {
  ...
  const handleDownloadPDF = async () => {
    try {
      ...
      pdf.save(`${agencyName}_expenses_${new Date().toISOString().split('T')[0]}.pdf`);
    } catch (error) {
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100">
      <div className="p-6 border-b border-gray-100">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h2 className="text-xl font-semibold text-gray-900">Expense Analysis</h2>
            <p className="text-gray-600 mt-1">Track and manage agency expenses</p>
            ...
            {filteredExpenses.length === 0 ? (
              ...
              filteredExpenses.map((expense) => (
                <tr key={expense.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 text-sm text-gray-900">{expense.orderId}</td>
                  <td className="px-6 py-4 text-sm text-gray-900">{expense.customerName}</td>
                  <td className="px-6 py-4 text-sm text-gray-900">{expense.orderType}</td>

...
import { TrendingUp, Calendar, DollarSign, TrendingDown, PiggyBank } from 'lucide-react';
...
const RevenueCards: React.FC<RevenueCardsProps> = ({
  ...
  const agencyCards = [
    ...
    },
    {
      ...
      value: weeklyProfit,
      icon: PiggyBank,
      color: weeklyProfit >= 0 ? 'from-green-500 to-green-600' : 'from-red-500 to-red-600',
  ...
  return (
    <div className={`grid ${gridCols} gap-4 mb-8`}>
      {cards.map((card, index) => {
        ...
        return (
          <div key={index} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 hover:shadow-md transition-shadow duration-300">
            ...
            <div className="text-sm text-gray-500">
              {card.title.includes('Profit') ? 'Net result' : 'From completed orders'}
            </div>
          </div>
        );
      })}

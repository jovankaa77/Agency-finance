...
const DailyAnalysis: React.FC<DailyAnalysisProps> = ({ data, orders, expenses }) => {
  ...
  const summaryCards = [
    ...
    },
    {
      ...
      bgColor: totalProfit >= 0 ? 'bg-green-50 dark:bg-green-900/30' : 'bg-red-50 dark:bg-red-900/30'
    }
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {summaryCards.map((card, index) => {
          ...
          return (
            <div key={index} className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
              <div className="flex items-center justify-between mb-4">
                <div className={`p-3 rounded-xl ${card.bgColor}`}>
                  <Icon className={`h-6 w-6 ${card.color}`} />
                </div>
              </div>
              <div className="text-sm font-medium text-gray-600 dark:text-gray-400 mb-2">{card.title}</div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
          <div>
            <h3 className="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-2">Daily Financial Analysis</h3>
            <p className="text-gray-600 dark:text-gray-400">Revenue, expenses, and profit trends over time</p>
          </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
          ...
          <div className="mt-4 space-y-3">
            ...
            <div className="flex justify-between items-center pt-2 border-t border-gray-200 dark:border-gray-700">
              <span className="text-sm font-medium text-gray-900 dark:text-gray-100">Net Profit:</span>
              <span className={`font-bold ${selectedDateProfit >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
          ...
          <div className="space-y-3 max-h-64 overflow-y-auto">
            {selectedDateOrders.length === 0 ? (
              <p className="text-gray-500 dark:text-gray-400 text-sm">No orders for this date</p>
            ) : (
                ...
                <div key={order.id} className="p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-500">{order.orderType}</p>
                    </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
          ...
          <div className="space-y-3 max-h-64 overflow-y-auto">
            {selectedDateExpenseList.length === 0 ? (
              <p className="text-gray-500 dark:text-gray-400 text-sm">No expenses for this date</p>
            ) : (
              selectedDateExpenseList.map((expense) => (
                <div key={expense.id} className="p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                  <div className="flex justify-between items-start">
                    <div>
                      <p className="font-medium text-gray-900 dark:text-gray-100">{expense.orderId}</p>
                      <p className="text-sm text-gray-600 dark:text-gray-400">{expense.customerName}</p>
                      <p className="text-xs text-gray-500 dark:text-gray-500">{expense.orderType}</p>
                    </div>
